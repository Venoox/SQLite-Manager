...
void MainWindow::on_pushButton_clicked()
{
    ...
    /*query.exec("CREATE TABLE database ( name varchar(20), surname varchar(20), age int);");
    query.prepare("INSERT INTO database (name, surname, age) "
    ...
    query.exec("SELECT COUNT(EmployeeId) from employees");
    query.next();
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if(db.isOpen()){
        ...
        if(query.exec(sqlstring)) {
            ui->textEdit_2->setText("Query executed successfully: " + query.lastQuery() + " (" + QString::number(query.numRowsAffected()) + " rows affected)");
        }
        else {
